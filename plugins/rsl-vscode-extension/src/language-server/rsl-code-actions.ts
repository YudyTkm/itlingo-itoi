import { AstNode, CodeActionProvider, DocumentValidator, EOL, LangiumDocument, MaybePromise } from 'langium';
...
export class RslActionProvider implements CodeActionProvider {
   ...
   */
  useNewId(diagnostic: Diagnostic, document: LangiumDocument<AstNode>): CodeAction | undefined {
   ...
   * @returns A `CodeAction` object or `undefined`.
   */
  createMissingElementForLinkingError(diagnostic: Diagnostic, document: LangiumDocument<AstNode>): CodeAction | undefined {
    const range = diagnostic.range;
    const data = diagnostic.data;

    const wrongWord = data.refText;
    let desiredElement = data.property;
    const elementProperty: LinguisticRuleElementProperty = 'id';
  ...
  removeString(title: string, diagnostic: Diagnostic, document: LangiumDocument<AstNode>): CodeAction | undefined {
    ...
    if (diagnostic.data) {
      try {
        range = diagnostic.data as Range;
      } catch (e) {
        // ignore
      }
    }
