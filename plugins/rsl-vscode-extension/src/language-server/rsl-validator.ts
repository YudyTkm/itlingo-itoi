import { AstNode, EOL, ValidationAcceptor, ValidationChecks, findNodeForKeyword, findNodeForProperty, getContainerOfType } from 'langium';
import {
  RslAstType,
  ActiveFlow,
...
export class RslValidator {
  ...
  checkActiveFlowCondition(element: ActiveFlow, accept: ValidationAcceptor): void {
    ...
    if (type !== 'SequenceConditional') {
      let range: Range | undefined = this.getKeywordAndPropertyRange(element, 'condition', 'condition');
  ...
  checkActiveTaskParticipantTarget(element: ActiveTask, accept: ValidationAcceptor): void {
    ...
    if (!(type === 'Send' || type === 'Receive')) {
      let range: Range | undefined = this.getKeywordAndPropertyRange(element, 'participantExternal', 'participantTarget');
  ...
  checkConstraintTypeSubType(element: Constraint, accept: ValidationAcceptor): void {
    ...
    if (!subType.includes(type)) {
      let range: Range | undefined = this.getKeywordAndPropertyRange(element, ':', 'subType');
  ...
  checkNoCycleInActiveTaskPartOfHierarchy(element: ActiveTask, accept: ValidationAcceptor): void {
    ...
    if (partOfElement.ref?.name === element.name) {
      let range: Range | undefined = this.getKeywordAndPropertyRange(element, 'partOf', 'partOf');
  ...
  checkNoCycleInActorIsAHierarchy(element: Actor, accept: ValidationAcceptor): void {
    ...
    if (superElement.ref?.name === element.name) {
      let range: Range | undefined = this.getKeywordAndPropertyRange(element, 'isA', 'super');
  ...
  checkNoCycleInStepsHierarchy(element: Step, accept: ValidationAcceptor): void {
    ...
    if (nextElement.ref?.name === element.name) {
      let range: Range | undefined = this.getKeywordAndPropertyRange(element, 'nextStep', 'next');
  ...
  checkSystemRelationSrcTrgt(element: SystemsRelation, accept: ValidationAcceptor): void {
    ...
    if (element.target === element.source) {
      let range: Range | undefined = this.getKeywordAndPropertyRange(element, 'source', 'source');
   ...
   */
  private getKeywordAndPropertyRange(element: AstNode, keyword: string, property: string) {
    const keywordNode = findNodeForKeyword(element.$cstNode, keyword);
    const propertyNode = findNodeForProperty(element.$cstNode, property);

    let range: Range | undefined;

    if (keywordNode?.range && propertyNode?.range) {
      range = {
        start: keywordNode?.range.start,
        end: propertyNode?.range.end
      };
    }
    return range;
  }
